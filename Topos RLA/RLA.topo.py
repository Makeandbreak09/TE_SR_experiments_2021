...
class RLA(Topo):
    ...
    def dijkstra_computed(self):
        # Demand from 0 to 4
        build_str = ""
        for dst in ["6", "3", "1", "8", "9", "7", "4"]:
            nhlist = self.get_dijkstra_route_by_name("0", dst)
            for nh in nhlist:
                build_str += f" nexthop via {nh.nh} encap seg6 mode inline segs {{{dst}}} weight {int(100/len(nhlist))} "
        ...
        for nh in nhlist:
            build_str += f" nexthop via {nh.nh} weight {int(100/len(nhlist))} "
        ...
        build_str = ""
        for dst in ["4", "2", "0"]:
            nhlist = self.get_dijkstra_route_by_name("3", dst)
            for nh in nhlist:
                build_str += f" nexthop via {nh.nh} encap seg6 mode inline segs {{{dst}}} weight {int(100/len(nhlist))} "
        ...
        build_str = ""
        for dst in ["6", "0", "1"]:
            nhlist = self.get_dijkstra_route_by_name("4", dst)
            for nh in nhlist:
                build_str += f" nexthop via {nh.nh} encap seg6 mode inline segs {{{dst}}} weight {int(100/len(nhlist))} "
        ...
        # IPv6 multipath hash policy
        for n in range(10):
            self.add_command(str(n), "sysctl net.ipv6.fib_multipath_hash_policy=1")

topos = {'RLA': (lambda: RLA())}
