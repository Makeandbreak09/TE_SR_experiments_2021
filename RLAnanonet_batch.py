...
class Tests:
	def __init__(self):
		...
		self.TESTFILES = ['RLA.topo.sh', 'join.topo.sh','weights.topo.sh']
		self.JSON_FILES = ['RLA.json', 'joint.json', 'weights.json']
...
for i in range(1, 10):
	for script in tests.TESTFILES:
		if script == 'RLA.topo.sh':
			tests.MAX_NODE = 9
		else:
			tests.MAX_NODE = 4

		print(f"Check Nr.{i} for {script} started")
		...
		max_val = str(tests.get_results(script))
		print("MAX=" + max_val)

		# Write to result CSV
		with open(OUTPUT_FILE, "a") as result_file:
			print(tests.TEST_ID, ';', script, ';', max_val, file=result_file)
